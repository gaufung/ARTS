...
#### 1.2.3.1 创建进程和线程
Windows 内核 API 包含明确的创建进程和线程的函数，但是对于 Linux 只有任务(Task) 概念，不同的任务之间可以选择是否共享内存、文件句柄等
- fork: 复制当前进程；
- exec: 使用可执行映像覆盖当前可执行映像；
- clone: 创建子进程并且从指定位置开始，包含的 `flag` 参数可以指定创建新的线程。

#### 1.2.3.2 线程安全
多个线程同时方便全局变量，这将导致条件竞争问题。产生的原因是程序代码并不是原语操作，而多线程调度导致程序真正执行并不可控制，解决办法是在程序执行过程进行同步操作。
- 二元信号量
- 互斥量
- 读写锁
- 条件变量
...
**线程模型**
- 1:1 用户空间线程和内核线程一对一关系；
- N:1 多个用户空间线程对于一个内核线程；
- N:M 多个用户空间线程对应于多个内核线程。
# 2 目标文件
