...
内核是如何在文件中找到这个表呢？当然这些信息都可以从 `ELF` 头中获取，因为它在文件中的偏移都是零。完成这一步，内核查看所有类型是 `LOAD` 的条目。然后将它们装载到进程内存空间。
正如你看到上面所列的，这里有两个条目的类型是 `LOAD`，你也可以发现这两个节被包含在同一个 `segment` 中。
现代操作系统和处理器以页的形式管理内存，你的计算机内存被划分成若干个大小相同的块，当你的应用程序需要内存，然后操作系统会分配若干个页给应用程序。这样做除了高效管理内存，还可以提供更好的安全性。操作系统和内核可以为每一个页分配保护位。保护位指定了特定页是否只读、可以写或者可以执行。如果一个页的保护位设置为只读，那么就不能被修改，这样做的话可以防止有意或者无意的修改数据。
...
在我们的例子中，你可以发现它指向的动态链接器的路径是 `/lib64/ld-linux-x86-64.so.2`。和我们的执行文件一样，内核开始通过阅读头表来装载这些共享文件，找到它的 `segments` 然后在我们当前进程内存空间加载这个共享库。在静态连接可执行文件中，这一步是不需要的，内核直接将控制权交给我们的程序。在这里内核将控制权交给动态链接器然后将 `main` 函数的地址压入到被调用栈中，所以当动态链接器完成它的工作，它知道如何将控制权交出去。
...
让我们回过头查看程序加载，我们完成了大部分工作。内核已经加载了全部可加载的 `segment`, 已经启动动态链接器。剩下的就是唤醒 `main` 函数。在链接器完工作结束，所有的工作已经完成了，当我们调用 `main` 函数，我们从终端得到如下的输出。
