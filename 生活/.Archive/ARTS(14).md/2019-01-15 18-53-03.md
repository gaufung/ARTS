...
控制反转就是框架的通用的特色，但是这是由代价的，当你在debug的时候，会发现非常难以理解。所以我一般会避免使用它们除非必须使用它们，并不是说它们是不好的东西，而是我想它需要考虑更加直接的方式。
一个重要的区别是使用`Service Locator`，每一个`service`的使用者有一个`locator`的依赖。`locator`能够隐藏其他实现的依赖，但是你需要看到`locator`。所以`locator`和`injection`之间的选择就是是否依赖是个问题。
使用`DI`能够帮助你更容易的看到每一个成分所依赖的内容，使用`DI`你可以只需要查看注入机制，比如使用构造器，然后都是看到依赖。使用`SL`，你需要查看全部代码，来查看哪里调用了`locator`。现代IDE能够帮助很好的完成，但是这个依然不是很容易查看构造和属性
# 3 Tips
   - emacs 替换
