...
## 1.1 分析过程
当`A`取完全棋子后，如果还剩下`5`个棋子，那么不管`B`取的方式，`A`总能获胜。那么问题转换为共`21-5=16`个棋子，在`A`能够取完全部棋子。那么问题就变成递归的执行，最后变成最后`16-5-5-5=1`棋子，`A`第一个取棋子，在后边每一轮中，保证和`B`取的棋子数量之和为`5`即可。
如果最后棋子是一颗炸弹，谁最后一个抽中就失败，那么见识
