...
如果已知 `next[j-1]`，那么如何得到 `next[j]`的值呢，很简单如果 `next[j-1]` 是 `j-1` 位置的时候下一轮要比较的位置，如果 `needle[j] == needle[next[j-1]]` 那么 `next[j] = next[j-1] + 1`，否则继续往前找更小的子串来匹配，比较 `needle[j] == needle[next[next[j-1]]]`，知道 `next[0] == -1` ，因为设置了通配符，这次相等比较肯定会成功。
```go
