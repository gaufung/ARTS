...
## Version
目前几乎所有项目使用`git`进行代码管理，公司内部搭建了`gitlab`， 所有代码项目中，如果使用git push <tag>操作，将会触发自动化部署工作。因此在构建的过程中，可以通过`git tag`最新标签，作为改版本后，在运行程序过程中，通过`./a -v`显示程序的版本号和构建时间。这样避免用开发人员每次修改代码中的软件版本号，尽量做到自动化工作。项目版本控制使用的`git flow`工具，项目主要有两个分支：`master`和`develop`。主要开发工作使用`git flow`等相关命令进行切换分支，当往gitlab上提交分支的引起Jekenis编译，但是代码是从master分支上拉取代码，私以为
