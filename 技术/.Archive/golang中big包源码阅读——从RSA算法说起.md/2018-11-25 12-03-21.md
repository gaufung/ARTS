...
# 2 Golang中Big包
由于`RSA`算法在实现过程中需要很大（位数很多）的数据，所以没有使用`int`、`int32`、`int64`等数据类型，而是使用`math.big`包中提供的`Int`类型。除了`Int`类型，还定义了`Rat`,`Float`等相关类型，由于`Go`不支持操作符重载，定义的类型方法中，返回值通常也是`receiver`，所以在使用过程中，不需要定义一些变量保存结果，直接使用链式调用即可。
## 2.1 类型
