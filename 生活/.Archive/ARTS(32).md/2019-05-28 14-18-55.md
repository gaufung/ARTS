...
- `bss` 节和 `data` 节差不多但是它是包含程序中未初始化的数据。未初始化的数据比如 `int arr[100]` 的声明，它就包含在 `bss` 节中。有一点需要注意的是 `bss` 节和其他节不同，它只包含该节占用大小而没有其他内容任何东西。这样做的原因是在加载的时候，这些所需要分配空间的数据都会在这一节分配，这样就能
# 3 Tip
