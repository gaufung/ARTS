...
class TestApp(AsyncHTTPTestCase):
    ...
    @mock.patch("app.bing")
    @mock.patch("app.MainHandler.baidu")
    @mock.patch("app.MainHandler.google")
    @gen_test
    def test_get(self, google, baidu, bing):
        future = Future()
        future.set_result("100")
        baidu.return_value = future

        future1 = Future()
        future1.set_result("200")
        google.return_value = future1

        future2 = Future()
        future2.set_result("300")
        bing.return_value = future2


        resposne = yield self.http_client.fetch(self.get_url("/baidu"))
        print(resposne.body)
