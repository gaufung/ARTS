...
既然保证算法的时间复杂度为$O(n)$，那么基本上排除先排序再处理的方式。然后就需要借助`map`数据结构来保存信息。对于连续序列，只需要找到其中的一个元素，就可以通过挨个向上或者向下回溯来确定序列的长度，那么为了避免重复回溯，需要标识位确定是否被记录过，而`map`中的`key`为元素，`value`对应着改元素索引值，可以通过索引来
